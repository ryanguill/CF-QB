...
<cflock name="resultsxls" timeout="30" type="exclusive">
	<cfsilent>
	...
	<cfif fileExists(expandPath('results.xls'))>
		<cffile action="delete" file="#expandPath('data/temp/results.xls')#" />
	</cfif>
	
	<cffile action="write" file="#expandPath('data/temp/results.xls')#" output="" addnewline="no"/>
	...
	<cffile action="write" file="#expandPath('data/temp/results.xls')#" output="#variables.out#" addnewline="no"/>
</cflock>
	
<cfheader name="Content-Disposition" value="attachment; filename=results.xls"> 
<cfcontent type="text/xml" file="#expandPath('data/temp/results.xls')#">
